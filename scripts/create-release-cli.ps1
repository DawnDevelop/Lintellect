# create-release-cli.ps1
# Creates a new CLI release branch and prepares for release
...
function Show-Usage {
  Write-Host "Usage: .\create-release-cli.ps1 -Version <version>"
  Write-Host ""
  Write-Host "Creates a new CLI release branch and prepares for release"
  Write-Host ""
  Write-Host "Parameters:"
  Write-Host "  -Version    The version number (e.g., 2.1.0)"
  Write-Host ""
  Write-Host "Examples:"
  Write-Host "  .\create-release-cli.ps1 -Version 2.1.0"
  Write-Host "  .\create-release-cli.ps1 -Version 3.0.0"
  Write-Host ""
  Write-Host "The script will:"
  Write-Host "  1. Create a release branch: release/cli/v<version>"
  Write-Host "  2. Update version numbers in project files"
  Write-Host "  3. Update CHANGELOG.md with CLI section"
...
function Test-VersionFormat {
  ...
  if ($Version -notmatch '^\d+\.\d+\.\d+$') {
    ...
    Write-Error "Version must be in format: MAJOR.MINOR.PATCH (e.g., 2.1.0)"
    exit 1
  }
...
function New-ReleaseBranch {
  ...
  $branchName = "release/cli/v$Version"
...
function Update-CliVersion {
  ...
  $projectFile = "src/Lintellect.Cli/Lintellect.Cli.csproj"
    
  Write-Status "Updating CLI version to $Version in $projectFile"
  ...
  if ($content -match '<PackageVersion>.*</PackageVersion>') {
    ...
    Write-Error "PackageVersion tag not found in $projectFile"
    exit 1
  }
  ...
  Write-Success "Updated CLI version to $Version"
}
...
function Update-Changelog {
  ...
  Write-Status "Updating CHANGELOG.md with CLI v$Version"
...
"@
  ...
  Write-Success "Updated CHANGELOG.md with CLI v$Version section"
...
function New-Commit {
  ...
  Write-Status "Creating commit for CLI v$Version release preparation"
    
  git add src/Lintellect.Cli/Lintellect.Cli.csproj CHANGELOG.md
    
  $commitMessage = @"
chore(cli): prepare release v$Version

- Update CLI version to $Version
- Add CLI v$Version section to CHANGELOG.md

This commit prepares the CLI for release v$Version.
...
"@
  ...
  Write-Success "Created commit for CLI v$Version"
}
...
function Show-NextSteps {
  ...
  Write-Host ""
  Write-Success "CLI release preparation completed!"
  Write-Host ""
  Write-Host "Next steps:"
  Write-Host "1. Update the CHANGELOG.md with actual changes for CLI v$Version"
  ...
  Write-Host "   git tag -a cli/v$Version -m `"Release CLI v$Version`""
  Write-Host "   git push origin cli/v$Version"
  Write-Host "5. The release workflow will automatically:"
  Write-Host "   - Build and publish NuGet package"
