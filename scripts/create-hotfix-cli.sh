...
# create-hotfix-cli.sh
# Creates a new CLI hotfix branch for critical fixes
...
show_usage() {
    ...
    echo ""
    echo "Creates a new CLI hotfix branch for critical fixes"
    echo ""
    echo "Arguments:"
    echo "  version     The hotfix version number (e.g., 2.1.1)"
    echo "  description Optional description of the hotfix"
    echo ""
    echo "Examples:"
    echo "  $0 2.1.1"
    echo "  $0 2.1.1 \"Fix memory leak in large solution analysis\""
    echo ""
    echo "The script will:"
    echo "  1. Create a hotfix branch: hotfix/cli/<description>"
...
validate_version() {
    ...
    if [[ ! $version =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        ...
        print_error "Version must be in format: MAJOR.MINOR.PATCH (e.g., 2.1.1)"
        exit 1
    fi
...
create_hotfix_branch() {
    local version=$1
    local description=$2
    local branch_name="hotfix/cli/v$version"
    ...
    if [ -n "$description" ]; then
        ...
        branch_name="hotfix/cli/v$version-$safe_description"
    fi
...
update_cli_version() {
    local version=$1
    local project_file="src/Lintellect.Cli/Lintellect.Cli.csproj"
    
    print_status "Updating CLI version to $version in $project_file"
    ...
    if grep -q "<PackageVersion>" "$project_file"; then
        sed -i.bak "s/<PackageVersion>.*<\/PackageVersion>/<PackageVersion>$version<\/PackageVersion>/" "$project_file"
        rm "$project_file.bak"
    else
        print_error "PackageVersion tag not found in $project_file"
        exit 1
    fi
    
    print_success "Updated CLI version to $version"
}
...
update_changelog() {
    ...
    print_status "Updating CHANGELOG.md with CLI v$version hotfix"
    ...
    print_success "Updated CHANGELOG.md with CLI v$version hotfix section"
...
create_commit() {
    ...
    print_status "Creating commit for CLI v$version hotfix"
    
    git add src/Lintellect.Cli/Lintellect.Cli.csproj CHANGELOG.md
    
    local commit_message="hotfix(cli): prepare hotfix v$version"
    if [ -n "$description" ]; then
...
- Add CLI v$version hotfix section to CHANGELOG.md

This commit prepares the CLI for hotfix v$version.
Please review and update the changelog content before merging."
    
    git commit -m "$commit_message"
    
    print_success "Created commit for CLI v$version hotfix"
}
...
show_next_steps() {
    ...
    echo ""
    print_success "CLI hotfix preparation completed!"
    echo ""
    echo "Next steps:"
    echo "1. Implement the hotfix in the branch: $branch_name"
    echo "2. Update the CHANGELOG.md with actual changes for CLI v$version"
    ...
    echo "   git tag -a cli/v$version -m \"Hotfix CLI v$version\""
    echo "   git push origin cli/v$version"
