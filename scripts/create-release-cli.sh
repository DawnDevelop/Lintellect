...
show_usage() {
    ...
    echo ""
    echo "Creates a new CLI release branch and prepares for release"
    echo ""
    echo "Arguments:"
    echo "  version    The version number (e.g., 2.1.0)"
    echo ""
    echo "Examples:"
    echo "  $0 2.1.0"
    echo "  $0 3.0.0"
    ...
    echo "  1. Create a release branch: release/cli/v<version>"
    echo "  2. Update version numbers in project files"
    echo "  3. Update CHANGELOG.md with CLI section"
...
validate_version() {
    ...
    if [[ ! $version =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        ...
        print_error "Version must be in format: MAJOR.MINOR.PATCH (e.g., 2.1.0)"
        exit 1
    fi
...
create_release_branch() {
    local version=$1
    local branch_name="release/cli/v$version"
...
update_changelog() {
    ...
    print_status "Updating CHANGELOG.md with CLI v$version"
    ...
    # Create temporary file with new CLI section
    local temp_file=$(mktemp)
    
    # Add new CLI section after [Unreleased]
    ...
    print_success "Updated CHANGELOG.md with CLI v$version section"
...
create_commit() {
    ...
    print_status "Creating commit for CLI v$version release preparation"
...
Please review and update the changelog content before merging."
    ...
    print_success "Created commit for CLI v$version"
}
...
show_next_steps() {
    ...
    echo ""
    print_success "CLI release preparation completed!"
    echo ""
    echo "Next steps:"
    echo "1. Update the CHANGELOG.md with actual changes for CLI v$version"
    ...
    echo "   git tag -a cli/v$version -m \"Release CLI v$version\""
