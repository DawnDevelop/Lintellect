...
internal class CodeQLJavaScriptAnalyzer : CodeQLAnalyzerBase
{
    public override EProgrammingLanguage Language => EProgrammingLanguage.JavaScript;
    ...
    {
        return "node_modules,dist,build,coverage,.next,.nuxt,out";
    }
    ...
    protected override async Task<string> GenerateCodeQLDatabaseAsync(string solutionPath)
    {
        ...
        var (success, output, error) = await CodeQLExtensions.ExecuteGitHubCliCommandAsync(
            $"codeql database create {databasePath} --language={GetLanguageIdentifier()} --source-root={solutionDir} --extractor-option=javascript.skipDirs={skipDirs}", "CodeQL Database").ConfigureAwait(false);
