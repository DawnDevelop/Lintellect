...
internal class CodeQLSwiftAnalyzer : CodeQLAnalyzerBase
{
    public override EProgrammingLanguage Language => EProgrammingLanguage.Swift;
    ...
    {
        return ".build,Packages,*.xcodeproj,DerivedData,node_modules,coverage";
    }
    ...
    protected override async Task<string> GenerateCodeQLDatabaseAsync(string solutionPath)
    {
        ...
        var (success, output, error) = await CodeQLExtensions.ExecuteGitHubCliCommandAsync(
            $"codeql database create {databasePath} --language={GetLanguageIdentifier()} --source-root={solutionDir} --extractor-option=swift.skipDirs={skipDirs}", "CodeQL Database").ConfigureAwait(false);
